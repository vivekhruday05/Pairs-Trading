...
from pairs_trading.backtesting import BacktestParameters, PairBacktestEngine
...
from pairs_trading.risk_management import RiskParameters
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Download data, generate pair signals, apply risk controls, and backtest P&L."
    )
    ...
    parser.add_argument("--stop-loss-fraction", type=float, default=0.03)
    parser.add_argument("--max-holding-period", type=int, default=20)
    parser.add_argument("--var-window", type=int, default=30)
    parser.add_argument("--var-confidence", type=float, default=0.95)
    parser.add_argument("--max-var-exposure", type=float, default=0.02)
    parser.add_argument("--transaction-cost-rate", type=float, default=0.0005)
...
def main() -> None:
    ...
    signal_frame = signal_result.to_frame()

    print("\n=== Risk controls and backtest phase ===")
    ...
    backtest_result = backtest_engine.run(
        ...
        risk_parameters=RiskParameters(
            ...
            max_var_exposure=args.max_var_exposure,
        ),
    ...
    backtest_frame = backtest_result.to_frame()
    ...
    if args.output:
        ...
        backtest_frame.to_csv(output_path, index=True)
        print(f"\nSaved backtest table to: {output_path}")
